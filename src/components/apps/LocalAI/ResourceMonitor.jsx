...
import { 
  Cpu, HardDrive, BarChart2, 
  TrendingUp, AlertCircle 
} from 'lucide-react';

export function ResourceMonitor({ settings }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setMetrics({
        cpu: Math.random() * 100,
        memory: (settings.maxMemory * Math.random()) / 1024,
  ...
  return (
    <div className="w-64 border-l border-gray-800 p-4">
      <h3 className="text-sm font-medium text-gray-400 mb-4">Resource Monitor</h3>
        ...
        <div>
          ...
          <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
            <div
              className="h-full bg-blue-500 transition-all duration-300"
              style={{ 
                width: `${(metrics.memory / (settings.maxMemory / 1024)) * 100}%` 
              }}
            />
          </div>
        </div>
